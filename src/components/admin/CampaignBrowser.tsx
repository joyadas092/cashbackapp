...
export function CampaignBrowser({
  ...
  if (!campaigns) {
    return <p className="text-sm text-white/50">Loading campaigns from Cuelinks...</p>;
  }
        ...
        <Card key={c.campaignId} className="p-4">
          <div className="flex items-center gap-3">
            {c.imageUrl ? (
              ...
              <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-white/10 text-xs text-white/40">
                No logo
              </div>
            )}
            ...
            {c.imported ? (
              <span className="rounded-full bg-cashlime-500/15 px-3 py-1 text-xs font-semibold text-cashlime-400">
                Imported
              </span>
            ) : (
...
function LinkForm({
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-4 flex flex-wrap items-end gap-3 border-t border-white/10 pt-4">
      <label className="flex flex-col gap-1 text-xs text-white/60">
        ...
        <select
          ...
          required
          className="rounded-full border border-white/15 bg-navy-900/80 px-4 py-2 text-sm text-white outline-none focus:border-violet-500"
        >
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-4 flex flex-col gap-3 border-t border-white/10 pt-4">
      <div className="grid grid-cols-2 gap-3">
        ...
        <label className="col-span-2 flex flex-col gap-1 text-xs text-white/60">
